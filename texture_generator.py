...
fileEx = userinput+".png"
source = Image.open(fileEx)
...
align = open(os.path.dirname(os.path.realpath(__file__)) + "\\alignments.json", "r")
...
while j < 47:
    ...
    if aligns[str(j)]["bl"] == "cr":
        i = 0
        while i < math.floor((height*height)/4):
            temp[j][i % (math.floor(height/2)), (math.floor(height/2)) + math.floor(i/(math.floor(height/2)))] = crpx[i % (math.floor(height/2)), (math.floor(height/2)-1) - math.floor(i/(math.floor(height/2)))]
            i += 1
